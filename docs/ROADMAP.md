...
_None — next milestone (m19-promote-policy, carrying NEW-3 + CARRY-D4 + CARRY-D2 port-v2 amendments) will be defined when triggered by user request._

## Recently Closed Milestones

- **m18-m8-release-engine-adapters** (v0.13.0, 2026-08-28, d5c04d9) — M8 slice 2: real production adapters for the m17 `:release-engine` ports — new `:release-engine-adapters` module (FARCH-017-compliant: deps only `:release-engine` + `:foundation` + JGit) with `JGitScmSource` (real checkout/tag/push over bare repos incl. `RemoteRefUpdate` status inspection), `LocalFSArtifactRepository` (atomic publish, sha256 sibling verification), `EnvSecretResolver` (env-backed, no material in surfaces), `GitTagBasedReleaseManager` (composed orchestration) + `ReleaseEngineWiring.of`. TCK oracle completed: m17 contract classes made executable under JUnit 5.11 (suspend-@Test discovery gap fixed), property-based expectation hooks so fakes (13/13/9) and reals (12/12/8) run the same invariants, spec-v6 applicability matrix (10 fake-only invariants, override+gate-set guards), persisted push-arrival test (bare-side resolve kills no-op regressions). `:foundation` gains kotlin-reflect-backed SecretValue privacy guards (S25/S26); ADR-0027 (SecretValue.marker privacy-positive, closes LOW-D2); LOW-D1 closed via contract-class extraction (−815 LOC net, FIND-OVERENG-001 resolved). 16 commits since base `583c185`, full workspace 594 declared / 585 executed / 0 failed / 9 matrix skips; verify history: round-1 FAIL (CRIT-1 swallowed checkout errors + CRIT-2 no-op push, both caught by mutation probes), round-3 PASS_WITH_WARNINGS (HIGH-1 echo-only push tests closed); debt **PASS_WITH_WARNINGS** (0 CRITICAL/HIGH/MEDIUM introduced, 5 LOW introduced, 3 m17 carries re-emitted). Carries to m19-promote-policy: NEW-3 (Failure.toString sanitization), CARRY-D4 (approval gate on PromoteRequest), CARRY-D2 (push credential resolution) — the port-v2 amendment package.
  Cycle artifacts in `~/.local/share/sddk/projects/p-4c8272c9e7dcdfa2/cycle-artifacts/p-4c8272c9e7dcdfa2/m18-m8-release-engine-adapters/`.

- **m17-m8-capabilities-shell** (v0.12.0, 2026-08-28, 3197d39) — M8 first-slice shell: `:release-engine` module with ScmSource / ArtifactRepository / ReleaseManager segregated ports + scripted fakes + TCK contract suites (6 invariants each incl. secret-exclusion probes); `:foundation` gains `SecretRef` / `SecretValue` / `SecretResolver` and `CapabilityId` / `CapabilityDescriptor` primitives; FARCH-017 (`:release-engine` does not import real secret stores) and FARCH-018 (defensive source scan for credential-shaped literals in `release-engine/*` + `foundation/secret/*`) both backed by red-green self-tests. 13 commits since base `65650b7`, 155/0 tests green in target modules (80 `:foundation:test` + 56 `:release-engine:test` + 19 `:architecture-tests:test`; full workspace 507/0); debt verdict **PASS** (0 CRITICAL / 0 HIGH / 2 MEDIUM / 2 LOW — Decision Contract row 7 applies; both MEDIUM are security-adjacent coupling concerns flagged as P2 backlog). 3-round adversarial verify history: round-1 FAIL (CRIT-A FARCH-018 no-op + CRIT-B tautological TCK exclusion), round-2 FAIL (CRIT-A path bug + CRIT-B vacuous marker), round-3 PASS (both CRITs closed with empirical red-green probes; NEW-1 + NEW-2 closed).
  New in m17: `:release-engine` module created (`release-engine/build.gradle.kts`, settings include); `foundation/secret/SecretRef.kt` + `SecretValue.kt` + `SecretResolver.kt` + `SecretFailure.kt`; `foundation/capability/CapabilityId.kt` + `CapabilityDescriptor.kt`; `release-engine/scm/ScmSource.kt` + `ScmFailure.kt` + `testing/FakeScmSource.kt` + `contract/ScmSourceContractTest.kt` (6 invariants incl. secret-exclusion probe); `release-engine/artifact/ArtifactRepository.kt` + `ArtifactFailure.kt` + `testing/FakeArtifactRepository.kt` + `contract/ArtifactRepositoryContractTest.kt` (6 invariants incl. secret-exclusion probe); `release-engine/release/ReleaseManager.kt` + `ReleaseFailure.kt` + `testing/FakeReleaseManager.kt` + `contract/ReleaseManagerContractTest.kt` (6 invariants incl. secret-exclusion probe); FARCH-017 `:release-engine` dependency-boundary rule; FARCH-018 `SecretIsolationSourceScanTest.kt` defensive source scan over `release-engine/*` + `foundation/secret/*` with red-green self-test (`PROBE-SECRET-MATERIAL-…` marker must be detected). Empty `release-engine/src/main/.../release/contract/` directory (LOW-D1 spec-vs-impl deviation) and `SecretValue.marker` privacy-positive private-only deviation (LOW-D2 ADR candidate) are both documented in verify-report-r3 and flagged as m18a backlog.
  Cycle artifacts in `~/.local/share/sddk/projects/p-4c8272c9e7dcdfa2/cycle-artifacts/p-4c8272c9e7dcdfa2/m17-m8-capabilities-shell/`.
